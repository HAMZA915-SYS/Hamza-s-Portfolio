...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dot && ring && window.matchMedia('(hover:hover)').matches) {
    ...
    animCursor();
    document.querySelectorAll('a, button, .service-card, .project-card, .about-card, .wm-card').forEach(el => {
  ...
  if (canvas) {
    ...
    const blobs = [
      { x:.25, y:.3,  r:.55, c:'rgba(0,229,255,', spd:.0005, phase:0   },
      { x:.7,  y:.6,  r:.5,  c:'rgba(168,85,247,', spd:.0007, phase:2  },
      { x:.5,  y:.8,  r:.45, c:'rgba(0,255,136,',  spd:.0004, phase:4  },
      { x:.1,  y:.7,  r:.4,  c:'rgba(59,130,246,',  spd:.0006, phase:1 },
    ];

    function draw() {
      ctx.clearRect(0, 0, W, H);
      blobs.forEach(b => {
        const x  = (b.x + Math.sin(t * b.spd * 2 + b.phase) * .15) * W;
        const y  = (b.y + Math.cos(t * b.spd * 1.5 + b.phase) * .12) * H;
        const r  = b.r * Math.min(W, H);
        const pulse = 0.06 + 0.025 * Math.sin(t * b.spd * 3);
        const grd = ctx.createRadialGradient(x, y, 0, x, y, r);
        grd.addColorStop(0,   b.c + pulse + ')');
        grd.addColorStop(.6,  b.c + (pulse * .3) + ')');
  ...
  if (tw) {
    const words = ['Frontend Developer', 'Webflow Designer', 'UI Builder', 'Web Developer'];
  ...
  /* ── SCROLL ZOOM ───────────────────────────── */
  const zoomTargets = document.querySelectorAll('.about-section, .services-section, .skills-section, .projects-section, .whyme-section, .contact-section');
  ...
  zoomTargets.forEach(el => { el.classList.add('zoom-section'); zoomObs.observe(el); });
